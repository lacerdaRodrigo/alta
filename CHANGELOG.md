...
### Alterado
- **CPF deixou de ser obrigatório e de ser validado no cadastro de paciente.** Pedido explícito do cliente: nem todo paciente chega ao primeiro atendimento com o CPF em mãos, e o app não deve travar o cadastro por isso. `TelaCadastroPaciente` não checa mais o campo em `_listarCamposFaltando()` (campo vazio passa) nem chama `ValidadorCpf.validar()` (dígitos verificadores não são mais exigidos — qualquer sequência de até 11 dígitos é aceita, válida ou não). O rótulo do campo perdeu o `*` de obrigatório.
  - **Duplicidade continua bloqueada**, mas só quando o campo não está vazio — a comparação de CPF (`p.cpf` vs. o texto digitado, sem máscara) é puramente textual, então dois pacientes sem CPF não colidem entre si, mas dois pacientes com o **mesmo** CPF "inválido" (ex.: `11111111111` repetido) continuam sendo barrados igual a qualquer duplicata real.
  - `Validadores.validarCPF`/`ValidadorCpf.validar` (o algoritmo em si) não foram tocados — continuam disponíveis e testados como utilitário; só pararam de ser chamados nesta tela.
  - `test/widgets/telas/tela_cadastro_paciente_test.dart`: CT-F1/CT-F2/CT-F4 ajustados (CPF não conta mais como pendência), CT-F3 reescrito para testar exatamente isso, e 3 novos testes cobrindo CPF com sequência sem checksum válido, CPF em branco não gerando falso positivo de duplicata, e CPF duplicado sendo barrado mesmo quando "inválido".
  - Total de testes: **365 → 368** (`tela_cadastro_paciente_test.dart` 23 → 26).
